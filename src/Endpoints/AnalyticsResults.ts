...
export class PublicResults {
    ...
    static ReturnAvailableVersions(req: http.IncomingMessage, res: http.ServerResponse<http.IncomingMessage>) {
        try {
            const apiKey = Utils.GetHeader(req, 'apiKey');
            if (apiKey == "") {
                res.statusCode = 406;
            ...
            let items = MainDB.getOperationVersions();
    ...
    static async GenerateReport_CurrentInstant(req: http.IncomingMessage, res: http.ServerResponse<http.IncomingMessage>) {
        ...
        const apiKey = Utils.GetHeader(req, 'apiKey');
        const op_version = Utils.GetHeader(req, 'operationVersion');
        if (apiKey == "") {
            res.statusCode = 406;
            return;
        }
        ...
        const report = await MainDB.GenerateReport(10, op_version);
